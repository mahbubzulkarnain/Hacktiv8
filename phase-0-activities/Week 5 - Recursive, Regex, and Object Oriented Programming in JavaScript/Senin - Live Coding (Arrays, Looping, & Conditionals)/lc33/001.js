 ...
 * READ and SAVE 'input'
 * 
 * SET 'isPrima' equal TRUE
 * SET 'result' equal ARRAY OF NULL
 * 
 * SET i equal 2
 * FOR i UNTIL i <= input
 *    IF i >= 2 THAN
 ...
 *        FOR 'j' UNTIL 'j' < 'i' 
 *            IF 'i' % 'j' equal 0 THAN
 *                SET 'isPrima' equal FALSE
 *            ENDIF
 *            SET 'j' equal 'j' plus 1
 *        ENDFOR
 *    ENDIF
 *    IF 'isPrima' equal TRUE THAN
 *        DISPLAY 'i'
 *    ENDIF
 *    SET 'i' equal 'i' plus 1
 * ENDFOR
 */
...
function getPrime(num) {
  // your code here
  let result = []
  if(num<1 || typeof num != 'number') return result
  let isPrima = true
  for (let i=2; i<=num; i++){
    if(i>=2){
      isPrima =true;
