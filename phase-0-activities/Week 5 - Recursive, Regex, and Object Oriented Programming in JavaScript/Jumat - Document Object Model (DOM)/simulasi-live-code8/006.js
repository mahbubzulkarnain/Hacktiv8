...
function consonantCounterRecursive(sentences) {
    // your code here
    const kamus = 'bcdfghjklmnpqrstvwxyz';
    if (sentences.length < 1) return sentences;
    let isFound = false;
    for (let i = 0; i < kamus.length; i++) {
        if (kamus[i].toLowerCase() === sentences[0] || kamus[i].toUpperCase() === sentences[0]) {
            isFound = true;
            break;
        }
    }
    return parseInt((isFound ? 1 : 0) + consonantCounterRecursive(sentences.slice(1)));
}
